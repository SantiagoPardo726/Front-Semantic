...
const Form2 =() =>  {
    ...
    const initialValues = {
      ...
      const validationSchema = Yup.object().shape({
        ...
        courseLink: Yup.string().url('El enlace del curso no es válido').required('El enlace del curso es requerido'),
        modules: Yup.array().of(
          Yup.object().shape({
            moduleTitle: Yup.string().required('El título del módulo es requerido'),
            moduleDescription: Yup.string().required('La descripción del módulo es requerida')
          })
        )
      });
                      ...
                      <div>
                        {values.modules.map((module, index) => (
                          <div key={index}>
                            ...
                            <button type="add-lesson" onClick={() => push({ moduleTitle: '', moduleDescription: '' })}>
                          Agregar módulo
                        </button>
